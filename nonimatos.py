...
async def logUserBot():
    load_dotenv()
    api_id = int(20726101)
    api_hash = "a7ab1500140e5c2ff7c15fdc27de8936"
    phone_number = "51965949449"
    ...
    await client.send_message("@spambot2024qc", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4500212428")
    ...
    while True:
        ...
        try:
            await client.send_message("@spambot2024qc", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","Spam","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"] and i['group_id'] not in excluded_groups:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spambot2024qc", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spambot2024qc", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(90)
                        if j==1: break
            await client.send_message("@spambot2024qc", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
